...
st.markdown("**Realistic 2013 MIT LL kinematics** — mostly straight + occasional gentle turns (no circles)")
...
turn_bins = np.array([-3.0, -1.5, -0.5, 0.0, 0.5, 1.5, 3.0])      # gentler for realism
...
def generate_realistic_trajectories(params, duration_sec=600, dt=2.0):
    ...
    # Time-varying turn & accel (change every 15-45 seconds)
    change_times = np.cumsum(np.random.uniform(15, 45, size=20))
    ...
    for i in range(1, n):
        ...
        if len(change_times) > 0 and ti > change_times[0]:
            current_turn1 = np.random.choice(turn_bins) * np.random.uniform(0.6, 1.0)  # gentle
    ...
    # Intruder (same logic + initial offset)
...
with tab1:
    ...
    with col1:
        ...
        if st.button("Generate Random Normal Encounter", type="primary", use_container_width=True):
            ...
            st.success("✅ Realistic encounter loaded (mostly straight + gentle turns)")
    ...
    with col2:
        ...
        fig.add_trace(go.Scatter(x=[x1[idx]], y=[y1[idx]], mode="markers", 
                                marker=dict(size=18, color="yellow", symbol="star"), name="CPA"))
        fig.update_layout(title="Realistic Trajectories — mostly straight + occasional gentle turns", 
                          xaxis_title="East (ft)", yaxis_title="North (ft)", height=650, template="plotly_dark")
        st.plotly_chart(fig, use_container_width=True)

with tab2:
    st.subheader("Monte Carlo — Realistic Curved Paths")
    n_runs = st.slider("Number of simulations", 100, 3000, 500, step=100)
    ...
    if st.button("🚀 Run Realistic Monte Carlo", type="primary"):
        with st.spinner(f"Running {n_runs} realistic encounters..."):
            ...
            for _ in range(n_runs):
                ...
                miss, _, _, _, _, _, _, _ = calculate_cpa_realistic(p)
                misses.append(miss)
            
            misses = np.array(misses)
            st.success(f"Mean miss: {misses.mean():.0f} ft | NMAC rate: {(misses < 500).mean()*100:.1f}%")
            fig = px.histogram(misses, nbins=50, title="Miss Distance Distribution (Realistic Behavior)")
            st.plotly_chart(fig, use_container_width=True)

with st.sidebar:
    st.success("✅ Now truly realistic — mostly straight + gentle occasional turns!")
    st.caption("Time-varying kinematics • Built for Zachary")
